...
function getLeaflet(lat,lng,zoom) {
 ...
 //
        ...
        //var sapelo_borrow_pit_gallery = L.marker([31.436370, -81.275273], {icon: L.AwesomeMarkers.icon({icon: 'video-camera', prefix: 'fa', markerColor: 'blue', spin:false}) }).bindPopup("Borrow Pit").addTo(images);
         ...
         //var sapelo_alligator_pond_gallery = L.marker([31.397869, -81.278431], {icon: L.AwesomeMarkers.icon({icon: 'video-camera', prefix: 'fa', markerColor: 'blue', spin:false}) }).bindPopup().addTo(images);
...
function getGallery(galleryName) {
   ...
   $.ajax({
        ...
        success: function(data) {
            ...
            return galleryName
            $("#"+galleryName).append("<div>testing</div>")
            console.log("text")
        },
        error: function() {
            alert('Error occured');
